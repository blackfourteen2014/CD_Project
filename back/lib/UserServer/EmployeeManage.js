...
router.post("/employeemanageuserlist", (req, res) => {
  ...
  let workTimeSum = 0;
  db.query(
    "SELECT * from employeeWork Join employee ON employee.id = employeeWork.id where Date = ?",
    [DateData],
    (error, userList) => {
...
router.post("/employeemanageusermonthlylistread", (req, res) => {
  ...
  const splitDate =
    req.body.SaveDate.split("/")[0] + "/" + req.body.SaveDate.split("/")[1];

  db.query(
    "SELECT * from employeeWork where id=? and Date like ?",
    [req.body.UserID, `${splitDate}%`],
    (error, userlist) => {
      if (error) throw error;
