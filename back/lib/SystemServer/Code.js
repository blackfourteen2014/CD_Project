...
router.post("/mastercodecreate", (req, res) => {
  db.query(
    `INSERT INTO mastercode(LargeCode,LargeInfo) VALUES(?,?)`,
    [req.body.LargeCode, req.body.LargeInfo],
    (error, result) => {
      if (error) {
        return res.json({
...
router.post("/mastercodedelete", (req, res) => {
  req.body.forEach((user) => {
    //console.log(user.id);
    db.query(
      `DELETE FROM mastercode WHERE LargeCode = ?`,
      [user.LargeCode],
...
router.post("/mastercodesearchlistread", (req, res) => {
  db.query(
    "SELECT * from MasterCode where LargeCode like ?",
    [`%${req.body.LargeCode}%`],
    (error, data) => {
      if (error) res.send([""]);
      db.query(
...
router.post("/smallcodecreate", (req, res) => {
  ...
  db.query(
    `INSERT INTO smallcode(SmallCode,SmallInfo,SmallContent) VALUES(?,?,?)`,
    [code, req.body.SmallInfo, req.body.SmallContent],
    (error, result) => {
      if (error) {
        return res.json({
...
router.post("/smallcodedelete", (req, res) => {
  req.body.forEach((user) => {
    db.query(
      `DELETE FROM smallcode WHERE SmallCode = ?`,
      [user.SmallCode],
      function (error, result) {
        if (error) {
          throw error;
