...
function ManageUpdate(props) {
  ...
  const handleOk = () => {
    ...
    if (Password === "") {
      ...
      //유저 데이터 Update
      dispatch(UserUpdate(body)).then((response) => {
  ...
  return (
    <>
      ...
      >
        <div>부서</div>
        <Select
          defaultValue={props.UserData.dept}
          style={{ width: 160 }}
          onChange={handleDept}
        >
          {DeptList.map((dept) => (
        ...
        <div>직급</div>
        <Select
          defaultValue={props.UserData.rank}
          style={{ width: 160 }}
          onChange={handleRank}
        >
          {RankList.map((rank) => (
        ...
        <div>사원번호</div>
        <Input placeholder="" value={Id} disabled={true} />
        <div>사원이름</div>
        ...
        <div>핸드폰번호</div>
        <Input placeholder="" value={Phone} onChange={handleChangePhone} />

        <div>우편번호</div>
        <Input placeholder="" value={Zim} onChange={handleChangeZim} />

        <div>주소</div>
        <Input placeholder="" value={Address} onChange={handleChangeAddress} />
