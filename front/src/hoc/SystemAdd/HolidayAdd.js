...
function HolidayAdd(props) {
  ...
  useEffect(() => {
    //휴일종류 코드 리스트 Read
    dispatch(HolyCodeListRead()).then((response) => {
  ...
  const handleOk = () => {
    ...
    //휴일 데이터 Create
    dispatch(HolidayCreate(body)).then((response) => {
  ...
  return (
    <>
      ...
      >
        <div style={{ fontSize: 15, background: "#fff" }}>
          날짜
          <Alert
            style={{ background: "#fff" }}
            message={moment(props.StartDate).format("YYYY/MM/DD")}
          />
        </div>
