...
function Code() {
  ...
  const handleDelete = () => {
    //소코드 데이터 Delete
    dispatch(SmallCodeDelete(CheckTarget)).then((response) => {
  ...
  function onChange(value) {
    if (value === "All") {
      //소코드 데이터 Read
      dispatch(SmallCodeRead()).then((response) => {
      ...
      dispatch(MasterCodeSearchListRead(body)).then((response) => {
        setSmallCode(response.payload);
      });
    }
  }
